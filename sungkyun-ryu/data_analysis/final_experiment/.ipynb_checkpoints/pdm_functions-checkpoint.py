...
# def get_unique_permutations(num_groups, num_permutations):
#     if num_permutations > math.factorial(num_groups):
#         raise ValueError("Number of requested permutations exceeds the total number of unique permutations.")
    
#     # Generate all possible permutations
#     all_permutations = list(permutations(range(num_groups)))
    
#     # Shuffle and sample the required number of permutations
#     selected_permutations = torch.randperm(len(all_permutations))[:num_permutations]
#     unique_permutations = [all_permutations[i] for i in selected_permutations.tolist()]

#     return unique_permutations

def multi_datasets(df, multi_dim, num_groups): 
    permuted_tensors_list = []
    num_rows = df.shape[0]
    num_cols = df.shape[1]
    grouped_tensor = df.view(num_rows, num_groups, -1)

    for _ in tqdm(range(multi_dim)): 
    ...
    final_tensor = stacked_tensor.view(-1, num_cols)

    return final_tensor

def multi_datasets_stacks(df, labels, multi_dim, num_groups): 
    permuted_tensors_list = []
    ...
    for _ in tqdm(range(multi_dim)): 
        permuted_indices = get_random_permutation(num_groups)
        permuted_tensor = grouped_tensor[:, :, permuted_indices, :, :]
    ...
    final_labels = labels.repeat(multi_dim)

    return final_tensor, final_labels

def multi_datasets_stacks_abnormal(df, multi_dim, num_groups, ab_indices, no_indices): 
    permuted_tensors_list = []
    label_list = []

    if ab_indices is not None:
        ab_df = df[ab_indices]
    
    num_dates = ab_df.shape[0]
    num_rows = ab_df[0][0].shape[0]
    num_features = ab_df[0][0][0].shape[0]
    grouped_tensor = ab_df.view(num_dates, 1 ,num_groups, int(num_rows/num_groups), -1)
    ...
    for _ in range(multi_dim): 
        ...
        label_list.extend([0] * len(permuted_tensor))
 
    stacked_ab_tensor = torch.cat(permuted_tensors_list, dim=0)
    normal_samples = df[no_indices]
    normal_labels = [1 for i in no_indices]

    final_tensor = stacked_ab_tensor.view(-1, 1, num_rows, num_features)
    final_tensor = torch.cat((final_tensor, normal_samples), dim=0)
    print(final_tensor.shape)

    final_labels = torch.tensor(label_list + normal_labels)

    return final_tensor, final_labels

    

# def get_random_selection(num_groups, select_count):
#     return torch.randperm(num_groups)[:select_count].tolist()
    
# def multi_datasets(df, multi_dim, num_groups, permute_groups): 
#     permuted_tensors_list = []
#     num_rows = df.shape[0]
...
#     for _ in range(multi_dim):  
#        selected_indices = get_random_selection(num_groups, permute_groups)        
#        permuted_indices = get_unique_permutations(permute_groups, multi_dim)        
#        permuted_tensor = grouped_tensor[:, permuted_indices, :]        
#        permuted_tensors_list.append(permuted_tensor)

#     stacked_tensor = torch.cat(permuted_tensors_list, dim=0)
#     final_tensor = stacked_tensor.view(-1, num_columns)
